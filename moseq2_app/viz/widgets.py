...
class SyllableLabelerWidgets:
    ...
    def __init__(self):
        ...
        self.lbl_name_input = widgets.Text(
            value="", placeholder="Syllable Name", tooltip="2 word name for syllable"
        )
...
class CrowdMovieCompareWidgets:
    ...
    def __init__(self):
        ...
        self.layout_visible = widgets.Layout(
            display="flex", align_items="stretch", justify_items="center"
        )

        self.cm_syll_select = widgets.Dropdown(
            options=[], description="Syllable #:", disabled=False
        )
        self.num_examples = widgets.IntSlider(
            value=20,
            min=1,
            max=100,
            step=1,
            description="# of Example Mice:",
            disabled=False,
        ...
        self.cm_sources_dropdown = widgets.Dropdown(
            options=["group", "SessionName", "SubjectName"],
            style=style,
            description="Movie Sources:",
        )

        self.cm_session_sel = widgets.SelectMultiple(
            options=[],
        ...
        self.cm_trigger_button = widgets.Button(
            description="Generate Movies",
            tooltip="Make Crowd Movies",
            layout=widgets.Layout(display="none", width="100%", align_items="stretch"),
        )

        self.syllable_box = VBox([self.cm_syll_select, self.num_examples])

        self.session_box = VBox(
            [self.cm_sources_dropdown, self.cm_session_sel, self.cm_trigger_button]
        )

        self.widget_box = HBox(
            [self.syllable_box, self.session_box],
