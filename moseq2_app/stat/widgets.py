...
class SyllableStatWidgets:
    ...
    def __init__(self):
        ...
        self.stat_dropdown = widgets.Dropdown(
            options=[
                "usage",
                "duration",
                "2D Velocity",
            ...
            description="Stat to Plot:",
            disabled=False,
        )

        self.sorting_dropdown = widgets.Dropdown(
            options=[
                "usage",
                "duration",
                "2D Velocity",
        ...
        self.ctrl_dropdown = widgets.Dropdown(
            options=[], description="Group 1:", disabled=False
        )
        self.exp_dropdown = widgets.Dropdown(
            options=[], description="Group 2:", disabled=False
        )

        self.grouping_dropdown = widgets.Dropdown(
            options=["group", "SessionName", "SubjectName"],
            description="Grouping:",
            disabled=False,
        )
        self.session_sel = widgets.SelectMultiple(
            options=[],
            description="Sessions:",
            rows=10,
            layout=self.layout_hidden,
            disabled=False,
        )

        self.errorbar_dropdown = widgets.Dropdown(
            options=["CI 95%", "None", "SEM", "STD"],
            description="Error Bars:",
            disabled=False,
        )

        self.hyp_test_dropdown = widgets.Dropdown(
            options=["KW & Dunn's", "Z-Test", "T-Test", "Mann-Whitney"],
        ...
        self.data_layout = widgets.Layout(
            flex_flow="row", padding="top", justify_content="space-around", width="100%"
        )

        self.stat_box = VBox([self.stat_dropdown])
        self.error_box = VBox([self.errorbar_dropdown])
        self.mutation_box = VBox(
            [self.ctrl_dropdown, self.exp_dropdown, self.hyp_test_dropdown]
        )

        self.sorting_box = VBox(
            [self.sorting_dropdown, self.mutation_box, self.thresholding_dropdown]
        )
        self.session_box = VBox([self.grouping_dropdown, self.session_sel])

        self.stat_widget_box = VBox(
            [
                HBox(
                    [
                        VBox([self.stat_box, self.error_box]),
                        self.sorting_box,
                        self.session_box,
                    ]
                )
...
class TransitionGraphWidgets:
    ...
    def __init__(self):
        ...
        col1_layout = widgets.Layout(
            flex_flow="column", width="75%", align_items="center"
        )
        col2_layout = widgets.Layout(
            flex_flow="column",
        ...
        self.graph_layout_dropdown = widgets.Dropdown(
        ...
        self.edge_thresholder = widgets.FloatRangeSlider(
            style=style,
            step=0.001,
            min=0,
            readout_format=".4f",
        ...
        self.usage_thresholder = widgets.FloatRangeSlider(
            style=style,
            step=0.001,
            min=0,
            readout_format=".3f",
        ...
        self.thresholding_box = HBox(
            [
                VBox(
                    [
                        self.graph_layout_dropdown,
                        self.edge_thresholder,
                        self.usage_thresholder,
                    ],
                    layout=col1_layout,
                ),
    ...
    def set_range_widget_values(self):
        ...
        usages_max = (
            ceil(max([max(normalize_usages(u).values()) for u in self.usages]) * 1000)
            / 1000.0
        )
