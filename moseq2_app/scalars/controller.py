...
class InteractiveScalarViewer(InteractiveScalarWidgets):
    ...
    def __init__(self, index_filepath):
        ...
        self.mean_df = self.scalar_df.groupby(
            ["uuid", "SessionName", "SubjectName", "group"], as_index=False
        ).mean()
        self.std_df = (
            self.scalar_df.groupby(
                ["uuid", "SessionName", "SubjectName", "group"], as_index=True
            )
            .std()
            .reset_index()
        )
    ...
    def make_graphs(self):
        ...
        for j, c in enumerate(selected_cols):
            for i, g in enumerate(unique_groups):
                ...
                texts = [
                    f"SessionName: {sn}<br>SubjectName: {sj}<br>uuid: {u}"
                    for sn, sj, u in zip(session_name, subject_name, uuid)
                ]
        ...
        self.fig.update_layout(
            height=300 * len(selected_cols), width=1000, title_text="Scalar Summary"
        )
