...
def get_scalar_anomaly_sessions(scalar_df, status_dicts):
    ...
    try:
        outliers = EllipticEnvelope(random_state=0).fit_predict(
            mean_df[val_keys].to_numpy()
        )
...
def print_validation_results(scalar_df, status_dicts):
    ...
    print(
        f"{bcolors.WARNING}{n_warnings}/{n_sessions} were flagged with warning(s).{bcolors.ENDC}"
    )
    print(
        f'Sessions with {bcolors.WARNING}"Warning"{bcolors.ENDC} flags may need visually inspection for outliers.\n'
    )
    ...
    for k in print_dict:
        ...
        print(
            f"{bcolors.BOLD}{bcolors.UNDERLINE}Session: {session_name}; Subject: {subject_name} flags:{bcolors.ENDC}"
        )
