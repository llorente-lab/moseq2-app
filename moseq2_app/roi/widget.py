...
_hover_dict = {
    ...
    "Frame (background subtracted)": HoverTool(
        tooltips=[("Height from floor (mm)", "@image")]
    ),
}
...
class ArenaMaskWidget:
    ...
    def __init__(
        ...
        overwrite_session_configs=True,
    ) -> None:
        ...
        if len(sessions) == 0:
            self.view = None
            print(
                "No sessions to show. There are either no sessions present or they are all extracted."
            )
            return
        ...
        if exists(session_config_path):
            ...
            if len(new_sessions) > 0:
                for _session in tqdm(
                    new_sessions, desc="Setting camera parameters", leave=False
                ):
            ...
            session_parameters = {
                basename(dirname(f)): detect_and_set_camera_parameters(
                    deepcopy(self.config_data), f
                )
                for f in tqdm(sessions, desc="Setting camera parameters", leave=False)
            }
            write_yaml(session_parameters, self.session_config_path)
        ...
        self.session_data = ArenaMaskData(
            path=list(self.sessions)[0], controller=self, configs=self.session_config
        )
        self.session_data.param.path.objects = list(
            self.sessions
        )  # generates object selector in gui
    ...
    def set_session_config_vars(self):
        ...
        session["bg_roi_weights"] = (
            self.session_data.mask_weight1,
            self.session_data.mask_weight2,
            self.session_data.mask_weight3,
        )
        ...
        session["flip_classifier_smoothing"] = (
            self.session_data.flip_classifier_smoothing
        )
        ...
        if isinstance(spatial_filter, (int, float)):
            ...
            print(
                "Incorrect spatial filter format. Spatial filter should be an integer or a list of integers. Setting to [3]",
                end="\r",
            )
            spatial_filter = [3]
        ...
        if isinstance(temporal_filter, (int, float)):
            ...
            print(
                "Incorrect temporal filter format. Temporal filter should be an integer or a list of integers. Setting to [0]",
                end="\r",
            )
            temporal_filter = [0]
        ...
        session["tracking_model_mask_threshold"] = (
            self.session_data.tracking_model_mask_thresh
        )
    ...
    def compute_arena_mask(self):
        ...
        strel_dilate = select_strel(
            session_config["bg_roi_shape"], tuple(session_config["bg_roi_dilate"])
        )
        strel_erode = select_strel(
            session_config["bg_roi_shape"], tuple(session_config["bg_roi_erode"])
        )
    ...
    def compute_extraction(self):
        ...
        frames = threshold_chunk(
            frames, session_config["min_height"], session_config["max_height"]
        )
...
class ArenaMaskData(param.Parameterized):
    ...
    depth_range = param.Range(
        default=(650, 750), bounds=(400, 1000), label="Floor depth range (mm)"
    )
    # defines how many iterations cv2.dilate should be run on mask computed for floor
    mask_dilations = param.Integer(
        default=3, bounds=(0, 10), label="Floor mask dilation iterations"
    )
    # defines thresholds used to locate mouse for extractions
    mouse_height = param.Range(
        default=(10, 110), bounds=(0, 175), label="Mouse height clip (mm)"
    )
    ...
    frames_to_extract = param.Integer(
        default=100, bounds=(1, None), label="Number of test frames to extract"
    )
    ...
    images = param.Dict(
        default={
            "Background": None,
            "Arena mask": None,
            "Extracted mouse": None,
            "Frame (background subtracted)": None,
        }
    )
    ...
    mask_shape = param.Selector(
        objects=["ellipse", "rectangle"], default="ellipse", label="Dilation shape"
    )
    dilation_kernel = param.Integer(
        default=10, bounds=(3, 40), label="Diameter of mask dilation kernel"
    )
    ...
    mask_weight3 = param.Number(
        default=1.0, bounds=(0, None), label="Mask distance from center"
    )

    mask_index = param.Integer(default=0, bounds=(0, None), label="Mask index")

    noise_tolerance = param.Number(
        default=30.0, bounds=(0, None), label="RANSAC noise tolerance"
    )

    ### advanced extraction parameters ###
    adv_extraction_flag = param.Boolean(label="Show advanced extraction parameters")
    crop_size = param.Integer(
        default=80, bounds=(1, None), label="Crop size (width and height; pixels)"
    )
    flip_classifier = param.Selector(
        label="Flip classifier"
    )  # TODO: fill this one out better
    flip_classifier_smoothing = param.Integer(
        default=51, bounds=(1, None), label="Flip classifier smoothing (frames)"
    )

    tracking_model_flag = param.Boolean(default=False, label="Use tracking model")
    tracking_model_mask_thresh = param.Number(
        default=-16, bounds=(None, 0), label="Tracking model mouse likelihood threshold"
    )

    cable_filters = param.Integer(
        default=0, bounds=(0, None), label="Cable filter iterations"
    )
    cable_filter_shape = param.Selector(
        objects=["rectangle", "ellipse"],
        default="rectangle",
        label="Cable filter shape",
    )
    cable_filter_size = param.Integer(
        default=5, bounds=(3, None), step=2, label="Cable filter size"
    )

    tail_filters = param.Integer(
        default=1, bounds=(0, None), label="Tail filter iterations"
    )
    tail_filter_shape = param.Selector(
        objects=["rectangle", "ellipse"], default="ellipse", label="Tail filter shape"
    )
    tail_filter_size = param.Integer(
        default=9, bounds=(3, None), step=2, label="Tail filter size"
    )
    ...
    chunk_overlap = param.Integer(
        default=0,
        bounds=(0, None),
        label="Extraction chunk overlap (useful for tracking model)",
    )
    frame_dtype = param.Selector(
        default="uint8", objects=["uint8", "uint16"], label="Output extraction datatype"
    )
    # NOTE: in the past this has been <i2, so might change some people's analysis
    movie_dtype = param.String(
        default="<u2", label="Raw depth video datatype (for .dat files)"
    )
    pixel_format = param.Selector(
        default="gray16le",
        objects=["gray16le", "gray16be"],
        label="Raw depth video datatype (for .avi or .mkv files)",
    )
    compress = param.Boolean(default=False, label="Compress raw data after extraction")

    ### flags and actions ###
    computing_arena = param.Boolean(
        default=False
    )  # used to indicate a computation is being performed
    computing_extraction = param.Boolean(
        default=False
    )  # used to indicate a computation is being performed
    # used to trigger the computation of the arena mask via a button
    compute_arena_mask = param.Action(
        lambda x: x.param.trigger("compute_arena_mask"), label="Compute arena mask"
    )
    # used to trigger the computation of a small extraction via a button
    compute_extraction = param.Action(
        lambda x: x.param.trigger("compute_extraction"), label="Compute extraction"
    )
    save_session_and_move_btn = param.Action(
        lambda x: x.param.trigger("save_session_and_move_btn"),
        label="Save session parameters and move to next",
    )
    save_session_btn = param.Action(
        lambda x: x.param.trigger("save_session_btn"), label="Save session parameters"
    )
    ...
    def set_config_data(self, session_path: str):
        ...
        self.mask_weight1, self.mask_weight2, self.mask_weight3 = session_config[
            "bg_roi_weights"
        ]
        ...
        self.tracking_model_mask_thresh = session_config[
    ...
    def change_frame_slider(self):
        ...
        self.param.frame_num.bounds = (
            0,
            min(self.frames_to_extract - 1, len(self.images["Extracted mouse"]) - 1),
        )
    ...
    def display(self):
        ...
        for k, v in self.images.items():
            if not isinstance(v, (np.ndarray, list)):
                ...
                im = hv.Image(
                    _img, label=k, bounds=(0, 0, v.shape[2], v.shape[1])
                ).opts(xlim=(0, v.shape[2]), ylim=(0, v.shape[1]), framewise=True)
            elif k == "Arena mask":
                _img = v[self.mask_index]
                im = hv.Image(
                    _img, label=k, bounds=(0, 0, _img.shape[1], _img.shape[0])
                ).opts(xlim=(0, _img.shape[1]), ylim=(0, _img.shape[0]), framewise=True)
            else:
                im = hv.Image(v, label=k, bounds=(0, 0, *v.shape[::-1])).opts(
                    xlim=(0, v.shape[1]), ylim=(0, v.shape[0]), framewise=True
                )
            panels.append(
                im.opts(
                    hv.opts.Image(
...
class ArenaMaskView(Viewer):
    ...
    def __init__(self, session_data: ArenaMaskData, **params):
        ...
        dilation_kernel = _link_data(
            pn.widgets.IntSlider, "dilation_kernel", step=1, height=40
        )
        ...
        noise_tolerance = _link_data(
            pn.widgets.NumberInput, "noise_tolerance", height=45
        )
        ...
        flip_classifier_smoothing = _link_data(
            pn.widgets.NumberInput, "flip_classifier_smoothing", height=40
        )
        tracking_model_flag = _link_data(
            pn.widgets.Checkbox, "tracking_model_flag", height=20
        )
        tracking_model_mask_thresh = _link_data(
            pn.widgets.NumberInput, "tracking_model_mask_thresh", height=40
        )

        cable_filters = _link_data(pn.widgets.IntInput, "cable_filters", height=40)
        cable_filter_shape = _link_data(
            pn.widgets.Select, "cable_filter_shape", height=40
        )
        cable_filter_size = _link_data(
            pn.widgets.IntInput, "cable_filter_size", height=50
        )

        tail_filters = _link_data(pn.widgets.IntInput, "tail_filters", height=40)
        tail_filter_shape = _link_data(
            pn.widgets.Select, "tail_filter_shape", height=40
        )
        tail_filter_size = _link_data(
            pn.widgets.IntInput, "tail_filter_size", height=40
        )
        ...
        save_session_and_move_btn = _link_data(
            pn.widgets.Button, "save_session_and_move_btn"
        )
        ...
        computing_check = _link_data(
            pn.widgets.Checkbox, "computing_arena", value=False, visible=False
        )
        ...
        computing_check.link(
            compute_mask_btn, callbacks={"value": _link_button_visibility}
        )
        ...
        computing_check2 = _link_data(
            pn.widgets.Checkbox, "computing_extraction", value=False, visible=False
        )
        computing_check2.link(indicator2, value="visible")
        computing_check2.link(
            compute_extraction_btn, callbacks={"value": _link_button_visibility}
        )
