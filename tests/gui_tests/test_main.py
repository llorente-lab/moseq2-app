...
from moseq2_app.main import (
    preview_extractions,
    validate_extractions,
    interactive_group_setting,
    label_syllables,
    interactive_syllable_stats,
    interactive_crowd_movie_comparison,
    interactive_transition_graph,
    view_extraction,
...
class TestMain(TestCase):
    ...
    def test_validate_extractions(self):
        ...
        extract_wrapper(
            "data/azure_test/nfov_test.mkv", None, config_data, num_frames=30, skip=True
        )
    ...
    def test_interactive_syllable_stat(self):
        ...
        interactive_syllable_stats(
            self.progress_paths, max_syllable=None, load_parquet=False
        )

        interactive_syllable_stats(
            self.progress_paths, max_syllable=5, load_parquet=True
        )
        ...
        interactive_syllable_stats(
            self.progress_paths, max_syllable=15, load_parquet=True
        )
    ...
    def test_interactive_crowd_movie_comparison_preview(self):
        ...
        label_syllables(self.progress_paths, max_syllables=None, n_explained=99)
        interactive_crowd_movie_comparison(
            self.progress_paths, group_movie_dir, get_pdfs=True, load_parquet=False
        )
        ...
        assert (
            len(
                [
                    f
                    for f in os.listdir(join(group_movie_dir, "default/"))
                    if f.endswith(".mp4")
                ]
            )
            == 1
